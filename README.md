...
- 撤销 / 重做
- 右键上下文菜单
- Ctrl+F 文内搜索（当前标签高亮匹配并支持上下跳转）
- JSON / YAML 文档支持：以带语法高亮的整文档代码块打开、编辑、保存，往返保留原始格式
- 代码块语言选择器：点击代码块右上角语言标签，手动设定/切换高亮语言

#### 多标签页
- **浏览器式标签管理** — 同时打开多个 Markdown 文件，每个文件独立标签
- **拖拽排序** — 按住标签拖动即可调整顺序，排序跨重启保留
...
- **拖拽多文件** — 一次拖多个 `.md` / `.markdown` / `.txt` / `.json` / `.yaml` / `.yml` 文件到窗口，各占一个标签
...
- 新建 / 打开 / 保存 / 另存为
- 导出 PDF
- 拖拽 `.md` / `.txt` / `.json` / `.yaml` 文件到窗口即可打开
- 二进制文件防护：`.zip` / 图片 / PDF 等改名成文本扩展名会被检测并拒绝打开，避免损坏
...
- Code-block language picker: click the language tag on a code block to set/switch its highlight language

#### Multi-Tab
- **Browser-style tab management** — open multiple Markdown files simultaneously, each in its own tab
- **Drag to reorder** — drag a tab to reorder; the order persists across restarts
...
- **Multi-file drop** — drag multiple `.md` / `.markdown` / `.txt` / `.json` / `.yaml` / `.yml` files into the window, each opens in its own tab
...
- Export to PDF
- Drag `.md` / `.txt` / `.json` / `.yaml` files to open
- Binary file guard: files like `.zip` / images / PDF renamed to a text extension are detected and rejected to prevent corruption
