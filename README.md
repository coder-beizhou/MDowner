...
- **持久化恢复** — 关闭软件再打开，上次所有标签和当前选中标签自动恢复
- **批量关闭检查** — 关闭窗口时检查所有标签修改状态，一键保存全部
...
#### 构建
```bash
npm run build:all    # 一键：打包 + 生成 zip + 清理（推荐）
npm run build:win    # 仅构建 portable + installer
```
...
- **Session restore** — all open tabs and active tab are restored when reopening the app
- **Batch close check** — closing the window checks all tabs for unsaved changes, with "Save All" option
...
#### Build
```bash
npm run build:all    # One-click: build + zip + cleanup (recommended)
npm run build:win    # Build portable + installer only
```
